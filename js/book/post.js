// Post a Listing tab: choosing/creating a PIN or security question,
// displaying the new ID, and the player/GM listing forms.
...
import { showInterstitial, showLeaves, setBookLabel, collectSystems, getPageLeft, getPageRight } from './book.js';

export function renderPostTab() {
  setBookLabel((state.book.code && state.book.postStage === 'details') ? ('✦ Posting as ' + state.book.code + ' ✦') : '✦ The Quest Board ✦');
  if (state.book.postStage === 'auth') { renderPostAuthSetup(); return; }
  if (state.book.postStage === 'newcode') { renderPostNewCode(); return; }
  renderPostDetails();
}
...
function renderPostAuthSetup() {
  ...
  document.getElementById('continueStepAuth').addEventListener('click', async () => {
    ...
    state.book.code = data;
    state.book.authValue = authValueToKeep;
    state.book.authType = authType;
    state.book.postStage = 'newcode';
    state.visitorCode = data;
    state.visitorAuthValue = authValueToKeep;
...
function renderPostNewCode() {
  showInterstitial(`
    ...
    <div class="code-display">${escapeHtml(state.book.code)}</div>
    <div class="modal-actions">
...
function bindPostForm(role, prefix, container) {
  ...
  container.querySelector(`#${prefix}_submit`).addEventListener('click', async () => {
    ...
    const { error } = await supabase.rpc('submit_listing_with_code', {
      p_code: state.book.code,
      p_auth_value: state.book.authValue,
      p_role: role,
      p_name: nameVal,
    ...
    state.book.myListings = null; // stale now, refetch next time submissions/interests is opened
    const label = role === 'dm' ? 'Game Master' : 'Player';
    container.innerHTML = `
      <span class="page-role-flag"><span class="dot"></span>${label}</span>
      <h2>Signed and sent</h2>
      <p class="page-sub">Pending review — usually within a day or two. Check "My submissions" with ID ${escapeHtml(state.book.code)} anytime.</p>
    `;
  });
}
...
function renderPostDetails() {
  showLeaves();
  const pageLeft = getPageLeft();
  const pageRight = getPageRight();
